...
for (x, y, w, h) in bounding_box:
    ...
    image_blurred = cv2.blur(gambar_wajah, ksize=(75, 75))
    cv2.imwrite("../output/hasil_crop_blur.jpg", image_blurred)
    image[y:(y+h), x:(x+w)] = image_blurred
    bounding_box_senyum = detector_senyum.detectMultiScale(box_wajah, 1.01, 15)
    if len(bounding_box_senyum) > 0:
        cv2.putText(image, "senyum terdeteksi", (x, y-10),
